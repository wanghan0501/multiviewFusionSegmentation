...
import datasets.transforms.multi_segment_transforms as MST
from datasets.multi_segment_dataset import MultiSegmentDataset
...
class Model:
    ...
    def __init__(self, config):
        ...
        self.multi_fusion = config['data']['multi_fusion']
    ...
    def run(self):
        ...
        eval_dataset = MultiSegmentDataset(
            data_root=data_params['data_root'],
        ...
        with tqdm(total=len(data_loader)) as pbar:
            for batch_id, sample in enumerate(data_loader):
                ...
                if self.multi_fusion:
                    batch, modality, channel, height, width = image.size()
                    image = image.view(batch * modality, channel, height, width)
